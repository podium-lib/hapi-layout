    ...
    register(server, layout) {
        ...
        server.ext({
            ...
            method: async (request, h) => {
                ...
                request.app.podium = await layout.context.process(req, res);
                return h.continue;
            },
        });

        // Mount proxy route
        const pathname = utils.pathnameBuilder(layout.httpProxy.pathname, layout.httpProxy.prefix, '/{path*}');
        server.route([
            {
                method: '*',
                path: pathname,
                handler: async (request, h) => {
                    const state = await layout.httpProxy.process(request.app.podium);
